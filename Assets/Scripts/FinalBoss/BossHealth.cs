...
public class BossHealth : MonoBehaviour
{
    ...
    // ====== PUBLIC API cho BossController ======

    public Action OnDeath;             // notify boss script (chuyển phase, cutscene,...)
    ...
    public void ApplySlow(float multiplier, float duration, bool allowStack)
    {
        ...
        if (flash != null)
            StartCoroutine(flash.FlashRoutine(slowFlashColor));
    }
    ...
    {
        if (damage <= 0) return;
        if (IsInvulnerable) return;   // boss đang bất khả xâm phạm
        ...
        {
            if (isDot)
                StartCoroutine(flash.FlashRoutine(dotFlashColor));
            else
    ...
    public void DetectDeath()
    {
        ...
        // báo cho BossController
        OnDeath?.Invoke();

        // multi-phase: set destroyOnDeath = false ở phase đầu
        if (!destroyOnDeath)
            return;
